...
export function AssetsView({ state, onOpenItem, onAdd, onAddDeployment }: { state: WorkspaceState; onOpenItem: (id: string) => void; onAdd: () => void; onAddDeployment: () => void }) {
      ...
      <section className="panel">
        ...
        <div className="asset-table asset-domain-table">
          ...
          {domains.map((asset) => {
            ...
            const days = daysUntil(asset.expiresAt);
            return <button className="asset-row" key={asset.id} onClick={() => item && onOpenItem(item.id)}>
              ...
              <span><b>{item?.name || "未关联产品"}</b><small>{asset.url || "可继续关联 DNS 与站点"}</small></span>
            </button>;
          })}
        ...
        <article className="panel">
          ...
          <div className="asset-card-list">
            {infrastructure.map((asset) => <article key={asset.id}>
              ...
              <em className={`asset-status status-${asset.status}`}>{statusLabels[asset.status]}</em>
            </article>)}
        ...
        <article className="panel">
          ...
          <div className="asset-card-list">
            {state.deployments.map((deployment) => {
              ...
              return <button key={deployment.id} onClick={() => item && onOpenItem(item.id)}>
                <span className="asset-kind">{deployment.role === "primary" ? "主力" : deployment.role === "testing" ? "测试" : "辅助"}</span>
                <div><strong>{deployment.name}</strong><small>{item?.name || "未知服务"} → {asset?.name || "未知节点"}{deployment.runtime ? ` · ${deployment.runtime}` : ""}</small></div>
                <em className={`asset-status status-${deployment.status}`}>{deployment.status === "online" ? "在线" : deployment.status === "degraded" ? "受限" : deployment.status === "offline" ? "离线" : "未知"}</em>
              </button>;
            })}
