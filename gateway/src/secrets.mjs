...
export function parseMasterKey(value) {
  ...
  if (!/^[A-Za-z0-9+/]{43}=$/.test(value)) {
    throw new Error("APIHUB_SECRETS_MASTER_KEY must be a base64-encoded 32-byte key");
  }
  const key = Buffer.from(value, "base64");
  if (key.length !== 32) throw new Error("APIHUB_SECRETS_MASTER_KEY must decode to exactly 32 bytes");
  return key;
}
