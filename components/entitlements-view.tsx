...
export function EntitlementsView({ state, onOpenItem, onAdd }: { state: WorkspaceState; onOpenItem: (id: string) => void; onAdd: () => void }) {
    ...
    <div className="view-stack">
      ...
      <section className="panel entitlement-table">
        ...
        {entries.map((entitlement) => {
          ...
          return (
            <button className="entitlement-row" key={entitlement.id} onClick={() => item && onOpenItem(item.id)}>
              <span className="product-cell"><i>{provider?.name.slice(0, 2) || "AI"}</i><span><strong>{item?.name || "未知产品"}</strong><small>{entitlement.label}</small></span></span>
              ...
              <span><b>¥{monthlyEquivalent(entitlement).toFixed(0)}</b><small>仅作成本概览</small></span>
              <span className={(days ?? 99) <= 30 ? "date-warning" : ""}><b>{nextDate || "未设置"}</b><small>{days === null ? "" : days < 0 ? `已过期 ${Math.abs(days)} 天` : `${days} 天后`}</small></span>
              <span className="link-pills">{links.map((link) => <em key={link.id}>{link.label}</em>)}{!links.length && <small>尚未关联入口</small>}</span>
            </button>
          );
        })}
      </section>
