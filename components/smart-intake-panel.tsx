...
export function SmartIntakePanel({ configured, serverMode, onOpenSettings, onCommitted }: {
  ...
  async function commitDraft() {
    ...
    try {
      ...
      setIssues([{ code: "committed", message: "订阅已经写入，目录、权益和可选发票已自动建立。" }]);
    } catch (error) {
  ...
  return <section className="smart-intake-panel">
    <div className="smart-intake-copy"><span className="kicker">NATURAL LANGUAGE INTAKE</span><h2>一句话录入订阅</h2><p>内容可以随意组织，不必按表单顺序。系统只生成草稿，确认后才会一次写入目录、权益和发票。</p></div>
    <div className="smart-intake-input">
      <textarea id="smart-intake-input" rows={3} maxLength={12000} value={message} onChange={(event) => { setMessage(event.target.value); if (draft) void closeDraft(true); }} placeholder="例如：新增 Qoder Pro，阿里巴巴，每月 20 美元，10 月 18 日续费、11 月 18 日到期，自动续费，提前 7 天提醒，官网信用卡购买" />
