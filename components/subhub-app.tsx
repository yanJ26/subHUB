...
export function SubHubApp() {
  ...
  return <div className="app-shell">
    ...
    <main className="main-content">
      <header className="topbar"><div><span className="kicker">SERVICES · RIGHTS · ASSETS</span><h1>{title}</h1></div><div className="topbar-actions"><label className="search-box"><span>⌕</span><input value={search} onChange={(event) => { setSearch(event.target.value); if (event.target.value) setView("catalog"); }} placeholder="搜索产品、厂商、角色或模型" /></label><button className="quick-button" onClick={() => setView("evaluations")}>✦ 快速记录</button></div></header>
      ...
      {view === "entitlements" && <EntitlementsView state={state} exchangeRates={exchangeRates} onOpenItem={setSelectedItemId} onAdd={() => setEditor({ kind: "entitlement" })} onEdit={(editId) => setEditor({ kind: "entitlement", editId })} />}
