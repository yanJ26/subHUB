...
当前支持三种明确意图：记录没有订阅关系的服务、新增订阅、修改已有订阅。模型没有工具、数据库或写入权限；删除、设置和其他意图会被拒绝或要求澄清。该入口是 subHUB 自身的 Web 功能，与 OpenClaw 无关。
...
- 用户明确表示“没有订阅、只是记录”时只能创建服务，不得伪造免费订阅；只有明确提到付费、试用、免费方案、按量 API、Token 包或购买关系时才能创建订阅记录。
- 普通代码再次验证真实日历日期、金额范围、枚举、HTTPS 链接、标签目录、重复方案和置信度。
- 录入统一使用一个日期字段 `renewsAt`（界面标注「到期 / 下次续费」）；底层 `expiresAt` 仅用于兼容旧的分离数据，写入新/改订阅时与 `renewsAt` 取同值。若一句话里出现两个明显矛盾的日期，会触发 `date_conflict` 风险标记并要求澄清，而不是让模型自行择一。
- 草稿绑定创建时的数据库 revision。其他窗口先修改数据后，旧草稿提交会返回冲突并要求重新生成。
- 草稿过期、取消或已提交后都不能再次写入。

## 自动建立的关系

确认订阅草稿后，Gateway 会在一个 SQLite 事务里：
...
确认“未订阅服务”草稿时只创建或复用 Provider 与 CatalogItem，并保存服务类型和使用状态；Entitlement、Invoice、费用、续费和到期记录保持为空。
