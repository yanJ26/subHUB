...
> 本文描述数据库和迁移协议，而不是日常页面的信息架构。用户界面只提供总览、服务和资产；Provider、CatalogItem、Entitlement 等关系由系统自动处理。没有 Entitlement 的 CatalogItem 就是“未订阅服务”。部署、入口、使用关系、额度、快照和评价目前只为既有数据兼容保留。
