...
subHUB 是一个私有的个人订阅与数字资产工具。它用三个页面管理大模型官方订阅、API、Agent 工具、域名、设备、服务器、账号、代码仓库和网站：**总览、订阅、资产**。

## 产品原则

- **一句话优先**：自由描述订阅，不要求先理解厂商、目录、权益和发票之间的内部关系。
- **订阅与资产分开**：Codex、Qoder 或 Cloudflare 是订阅的服务；`subhub.example.com`、电脑和 VPS 是自己控制的资产。
- **两个日期分开**：下次续费与权益到期分别记录，避免混淆扣款和实际可用期限。
...
- apiHUB 式自然语言录入：随意描述一项订阅，系统整理为可核对草稿，确认后一次保存服务、方案、费用、日期和可选发票。
...
模型只把这段话整理成固定结构，普通代码会再次检查日期、金额、枚举、标签、重复项和敏感内容。页面先显示草稿，只有 Owner 点击“确认写入”后才会在一个数据库事务中保存完整订阅及可选发票。原始自然语言不进入数据库，只保留 SHA-256 摘要。

“订阅”页保留手工一步式表单，但它只是后备入口。日常录入不需要先添加目录或建立其他关系。自然语言录入当前仅来自登录后的 Web 页面，与 OpenClaw 无关，也不依赖 OpenClaw。
