...
- 一步式订阅录入：一次提交服务、厂商、方案、费用、续费、到期和发票，后台自动创建或复用目录关系。
...
顶部“添加记录”或“使用权益”页的“添加订阅”是默认入口。普通订阅只需提交一次；系统会按厂商和服务名复用已有目录，并在同一事务中写入权益及可选发票。资产、部署、入口、额度和评价属于可选关系，仅在确有需要时补充，不是添加订阅的前置步骤。
